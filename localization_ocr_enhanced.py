    ...
    for track in tracks:
        ...
        if track_id not in results_dict:
            results_dict[track_id] = {
                ...
                "last_ocr_frame": 0,
                "ocr_attempts": 0,  # Track number of OCR attempts
                "max_ocr_attempts": 5  # Maximum attempts before giving up
            }
        
        # Check if we should attempt OCR or if we've exhausted attempts
        should_ocr = (
            results_dict[track_id]["ocr_attempts"] < results_dict[track_id]["max_ocr_attempts"] and
            len(results_dict[track_id]["all_detections"]) < 3 and  # Max 3 successful attempts only
        ...
        if track_id in results_dict:
            if results_dict[track_id]["best_plate_text"] == "NO_PLATE_FOUND":
                label += " | NO NUMBER PLATE FOUND"
                # Change box color to yellow for vehicles without readable plates
                cv2.rectangle(frame, (x1_expanded, y1_expanded), (x2_expanded, y2_expanded), (0, 255, 255), 2)
            elif results_dict[track_id]["best_plate_text"] and is_valid_indian_plate(results_dict[track_id]["best_plate_text"]):
...
def group_similar_plates(results_dict, similarity_threshold=0.75, min_confidence=0.8, min_group_size=2):
    ...
    for tid, info in results_dict.items():
        if (info["best_plate_text"] and 
            is_valid_indian_plate(info["best_plate_text"]) and 
            info["best_confidence"] >= min_confidence):
...
# Enhanced grouping with stricter parameters
print("\n[INFO] Grouping similar license plates with enhanced algorithm...")
final_results = group_similar_plates(
    results_dict, 
    similarity_threshold=0.75,  # Lower threshold for better grouping
    min_confidence=0.8,         # Higher minimum confidence
    min_group_size=2           # Require at least 2 detections per group
)
...
with open("detected_plates_final.json", "w") as f:
    json.dump(sorted_results, f, indent=4)

print("\n" + "="*80)
print("🚗 VEHICLES IN CHRONOLOGICAL ORDER (As they appeared in video)")
print("="*80)
...
# Enhanced summary
print(f"\033[94m{'='*80}")
print(f"📊 ENHANCED FINAL SUMMARY (Chronological Order):")
print(f"   🚗 High-confidence unique vehicles: {len(sorted_results)}")
print(f"   📋 Total valid detections: {len([tid for tid, info in results_dict.items() if info['best_plate_text'] and is_valid_indian_plate(info['best_plate_text'])])}")
print(f"   🎯 High-confidence detections: {len([tid for tid, info in results_dict.items() if info['best_plate_text'] and is_valid_indian_plate(info['best_plate_text']) and info['best_confidence'] >= 0.8])}")
print(f"   📈 Grouping efficiency: {len(sorted_results)} vehicles from {len([tid for tid, info in results_dict.items() if info['best_plate_text'] and is_valid_indian_plate(info['best_plate_text'])])} total detections")
...
print(f"\n\033[93m🕒 VEHICLES IN ORDER OF APPEARANCE:\033[0m")
for idx, (vehicle_id, info) in enumerate(sorted_results.items(), 1):
    fps = 30  # Adjust based on your video's FPS
    timestamp_seconds = info['frame_detected'] / fps
    minutes = int(timestamp_seconds // 60)
    seconds = int(timestamp_seconds % 60)
    print(f"   {idx}. {info['formatted_plate_text']} - appeared at {minutes:02d}:{seconds:02d} (frame {info['frame_detected']})")
...
with open("vehicle_timeline.json", "w") as f:
    json.dump(timeline_data, f, indent=4)

print(f"\n\033[96m💾 Timeline data saved to 'vehicle_timeline.json'\033[0m")
