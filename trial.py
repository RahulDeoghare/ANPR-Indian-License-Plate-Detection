...
def main():
    # --- Model Paths ---
    VEHICLE_MODEL = r"C:\Users\nidhi\Downloads\Front.pt"  # Change to your vehicle YOLO model
    PLATE_MODEL = "plate.pt"    # Change to your plate YOLO model
    VIDEO_SOURCE = r"C:\Users\nidhi\Videos\vms_test3.mp4"            # 0 for webcam, or path to video file
    ...
    # --- Tracking ---
    tracked_vehicles = {}  # {id: (bbox, has_crossed)}
        ...
        if roi_defined:
            ...
            for box in vehicle_results.boxes:
                ...
                if vehicle_id is None:
                    vehicle_id = next_id
                    next_id += 1
                has_crossed = tracked_vehicles.get(vehicle_id, (None, False))[1]
                current_vehicles[vehicle_id] = ((x1, y1, x2, y2), has_crossed)
            ...
            for plate_box in plate_results.boxes:
                ...
                if associated_vehicle_id is not None:
                    # Only OCR if not already done for this vehicle
                    if associated_vehicle_id not in last_plate_for_vehicle:
                        ...
                        # Save plate image and store result
                        ...
                        if best_text:
                            ...
                            formatted_plate = "NO_PLATE_FOUND"
                        last_plate_for_vehicle[associated_vehicle_id] = {
                            "plate_box": (px1, py1, px2, py2),
                            "ocr_text": formatted_plate,
                            "conf": best_confidence
                        }
                        print(f"[INFO] Vehicle {associated_vehicle_id}: Plate: {formatted_plate} (conf: {best_confidence:.2f})")
                # Draw plate box
            ...
            # --- Crossing Logic ---
            for vehicle_id, (bbox, has_crossed) in tracked_vehicles.items():
                ...
                if not has_crossed and not is_point_above_line(centroid, roi_points[0], roi_points[1]):
                    ...
                    if dist_to_line < 10:
                        crossing_count += 1
                        tracked_vehicles[vehicle_id] = (bbox, True)
                        # Show plate OCR result if available
